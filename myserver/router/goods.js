...
// 用户商品浏览记录接口
router.post('/goods/browsing/history', (req, res) => {
    const { goods_id, user_id, date } = req.body
    if (!goods_id || !user_id || !date) return
    userTable.find({ _id: user_id }).then(d => {
        if (!d[0].browsing_history.some(it => it.goods_id === goods_id)) {
            let arr = d[0].browsing_history
            arr.push({ goods_id, date })
            userTable.updateOne({ _id: user_id }, { $set: { browsing_history: arr } }).then(d1 => {
                res.send({ code: 200, msg: '你浏览了该商品' })
            })
        }
    })
})
